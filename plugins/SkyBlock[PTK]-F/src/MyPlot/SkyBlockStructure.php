...
class SkyBlockStructure extends Populator{
	...
	public static function placeObject(ChunkManager $level, $chunk, $Xofchunk, $Zofchunk){
		...
		$level->setBlockIdAt($vec->x + 9, 68, $vec->z + 8, Block::WATER);
		$level->setBlockIdAt($vec->x + 9, 68, $vec->z + 9, Block::WATER);
		$level->setBlockIdAt($vec->x + 9, 68, $vec->z + 7, Block::WATER);
		$level->setBlockIdAt($vec->x + 10, 69, $vec->z + 8, Block::REEDS);
		$level->setBlockIdAt($vec->x + 10, 68, $vec->z + 6, Block::FENCE);
		$level->setBlockIdAt($vec->x + 9, 68, $vec->z + 5, Block::FENCE);
		$level->setBlockIdAt($vec->x + 9, 68, $vec->z + 6, Block::COBBLESTONE);
		$level->setBlockIdAt($vec->x + 4, 69, $vec->z + 7, Block::CRAFTING_TABLE);
		$level->setBlockIdAt($vec->x + 4, 69, $vec->z + 6, Block::FURNACE);
